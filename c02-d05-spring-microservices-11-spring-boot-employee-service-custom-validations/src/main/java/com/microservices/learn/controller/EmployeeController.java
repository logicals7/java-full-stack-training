...
public class EmployeeController {
    ...
    @RequestMapping(value = "/emp/controller/addEmp", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<String> addEmployee(@Valid @RequestBody Employee employee, Errors err) {
        if (err.hasErrors()) {
            return new ResponseEntity<>(err.getAllErrors().toString(), HttpStatus.BAD_REQUEST);
        }
        int count = employeeDAO.addEmployee(employee);
        return new ResponseEntity<>("Employee added successfully with id:" + count, HttpStatus.CREATED);
    }

    @RequestMapping(value = "/emp/controller/updateEmp", method = RequestMethod.PUT, consumes = MediaType.APPLICATION_JSON_VALUE)
    ...
    public ResponseEntity<Employee> deleteEmployee(@PathVariable("id") int myId) {
        Employee existing = employeeDAO.getEmployeeDetailsById(myId);
        if (existing == null) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }
        Employee deleted = employeeDAO.removeEmployee(myId);
