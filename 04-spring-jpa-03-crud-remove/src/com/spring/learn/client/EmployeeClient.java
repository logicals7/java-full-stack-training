...
/*
	 ...
	 find():
		find() method will find the single record from db
		Will only work with primary key.
		Will return the entity type.
		Will accept the 2 parameters:
		Entity Class
		Primary Key


	remove():
		remove() method will only work with entity object.
		First we need to find the object using find() method
		Then we call the remove method by passing the entity object.
		For remove we will need to start and commit the transaction as well as JPA doesn’t auto-commit.
...
public class EmployeeClient {
	...
	public static void mergeOperation() {
		...
		entity.setEmpId(5); //it will be updated as the id 2 is already there in db in my case
